...
public class DunderOperatorImplementorTest {
    ...
    private BiFunction getMathFunction(PythonBytecodeInstruction.OpCode opCode) {
        PythonCompiledFunction pythonCompiledFunction = PythonFunctionBuilder.newFunction("a", "b")
                .loadParameter("a")
                .loadParameter("b")
                .op(opCode)
    ...
    public void testMathOp() {
        BiFunction javaFunction = getMathFunction(PythonBytecodeInstruction.OpCode.BINARY_ADD);

        assertThat(javaFunction.apply(1L, 2L)).isEqualTo(3L);

        javaFunction = getMathFunction(PythonBytecodeInstruction.OpCode.BINARY_SUBTRACT);
        assertThat(javaFunction.apply(3L, 2L)).isEqualTo(1L);

        javaFunction = getMathFunction(PythonBytecodeInstruction.OpCode.BINARY_TRUE_DIVIDE);
        assertThat(javaFunction.apply(3L, 2L)).isEqualTo(1.5d);

        javaFunction = getMathFunction(PythonBytecodeInstruction.OpCode.BINARY_FLOOR_DIVIDE);
        assertThat(javaFunction.apply(3L, 2L)).isEqualTo(1L);
    }
}
