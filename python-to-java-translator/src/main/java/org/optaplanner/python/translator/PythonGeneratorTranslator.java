...
import static org.optaplanner.python.translator.PythonBytecodeToJavaBytecodeTranslator.ANNOTATION_DIRECTORY_INSTANCE_FIELD_NAME;
import static org.optaplanner.python.translator.PythonBytecodeToJavaBytecodeTranslator.CELLS_INSTANCE_FIELD_NAME;
import static org.optaplanner.python.translator.PythonBytecodeToJavaBytecodeTranslator.DEFAULT_KEYWORD_ARGS_INSTANCE_FIELD_NAME;
import static org.optaplanner.python.translator.PythonBytecodeToJavaBytecodeTranslator.DEFAULT_POSITIONAL_ARGS_INSTANCE_FIELD_NAME;
...
public class PythonGeneratorTranslator {
    ...
    public static Class<?> translateGeneratorFunction(PythonCompiledFunction pythonCompiledFunction) {
        ...
        PythonBytecodeToJavaBytecodeTranslator.writeClassOutput(classNameToBytecode, className, classWriter.toByteArray());
        try {
            Class<?> out = asmClassLoader.loadClass(className);
    ...
    private static GeneratorMethodPart getGeneratorMethodPartForYield(String internalClassName,
            ...
            YieldValueOpcode yieldValueOpcode) {
        final String methodName = "advance" + (yieldValueOpcode.getBytecodeIndex() + 1);

        GeneratorMethodPart out = new GeneratorMethodPart();
        out.initialStackMetadata = stackMetadata;
        out.afterYield = yieldValueOpcode.getBytecodeIndex() + 1;
        ...
        functionMetadata.methodVisitor = MethodVisitorAdapters.adapt(functionMetadata.methodVisitor, methodName,
                Type.getMethodDescriptor(Type.VOID_TYPE));
        functionMetadata.pythonCompiledFunction = getPythonCompiledFunctionForYield(pythonCompiledFunction, yieldValueOpcode);
        return out;
    }

    private static PythonCompiledFunction getPythonCompiledFunctionForYield(PythonCompiledFunction original,
            YieldValueOpcode yieldValueOpcode) {
