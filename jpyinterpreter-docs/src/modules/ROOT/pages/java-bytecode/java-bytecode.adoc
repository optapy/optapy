...
- The stack is typed, and will cause verification errors if you try to call a method with invalid types on the stack.
For instance, this is invalid:
+
```
ILOAD 1
...
```
+
since you are calling a method that expects a `float` with an `int`.
...
```
+
Since you trying to call a `MyClass` method on `Object` (which may or may not be a `MyClass` instance).
To fix it, you would need to cast it first:
+
```
ALOAD 1 [Object]
CHECKCAST [MyClass]
...
adapting the method visitor with `MethodVisitorAdapters.adapt` (which sorts try blocks for you as well as give better error messages),
...
You can use the `LocalVariableHelper` on `StackMetadata` to obtain the slot number of parameters and local variables.
...
This can be gotten with `Type.getInternalName`.

** The method name.

** The method descriptor string that describe the parameter and return types of the method.
This can be received with `Type.getMethodDescriptor(Type returnType, Type... parameterTypes)` (you can get the `Type` object of a class with `Type.getType`).

** A boolean that true if the method is defined on an interface, false otherwise.

- Instructions that read or set fields are created with `visitFieldInsn`, which takes

** The internal name of the class this field belongs to. This can be gotten with `Type.getInternalName`.

** The field's name.

** The field's type descriptor string. This can be received with `Type.getDescriptor`.

- Instructions that operate on types (i.e. `CHECKCAST`, `INSTANCEOF` and `NEW`) are generated with `visitTypeInsn`. It takes the internal name of the class to cast/instanceof/new as an argument (this can be gotten with `Type.getInternalName`).
