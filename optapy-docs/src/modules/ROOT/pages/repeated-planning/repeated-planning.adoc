...
OptaPy allows you to start planning earlier, despite unforeseen changes, as the optimization algorithms support planning a solution that has already been partially planned. This is known as repeated planning.
...
By default, OptaPy assigns all planning entities, overloads the planning values, and therefore breaks hard constraints.
...
==== Pin down planning entities with `@planning_pin`

To pin some planning entities down, add an `@planning_pin` annotation on a bool getter of the planning entity class.
That boolean is `True` if the entity is pinned down to its current planning values and `False` otherwise.

. Add the `@planning_pin` annotation on a `bool`:
+
[source,python,options="nowrap"]
----
from optapy import planning_entity, planning_pin
...
In the example above, if `pinned` is `True`, the lecture will not be assigned to another period or room (even if the current period and rooms fields are `None`).


[[configureAPinningFilter]]
==== Configure a `PinningFilter`

Alternatively, to pin some planning entities down, create a predicate function that returns `True` if an entity is pinned, and `False` if it is movable.
This is more flexible and more verbose than the `@planning_pin` approach.
...
In the machine reassignment example, the entity has both the planning variable `machine` and its original value ``original_machine``:
...
def process_moved(constraint_factory: ConstraintFactory):
    return constraint_factory.forEach(ProcessAssignmentClass) \
               ...
               .penalize("process_moved", HardSoftScore.ofSoft(1000))
----
...
Notice the method returns `CompletableFuture[None]`, which is completed when a user-defined `Consumer` accepts
...
[source,python,options="nowrap"]
----
from optapy import problem_change
...
def delete_computer(computer: CloudComputer) -> None:
    solver.addProblemChange(DeleteComputerProblemChange(computer))
----

. Any change in a `@problem_change` must be done on the `working_solution` (the first parameter in the `doChange` method).

. The `working_solution` is xref:planner-configuration/planner-configuration.adoc#cloningASolution[a planning clone] of the ``BestSolutionChangedEvent``'s ``best_solution``.
* The `working_solution` in the `Solver` is never the same solution instance as in the rest of your application: it is a planning clone.
* A planning clone also clones the planning entities and planning entity collections.
+
Thus, any change on the planning entities must happen on the `working_solution` instance passed to the `ProblemChange.doChange(working_solution: Solution, problem_change_director: ProblemChangeDirector)` method.
...
This requires xref:optimization-algorithms/optimization-algorithms.adoc#planningId[annotating a property of that class as the @planning_id].

. A planning clone does not clone the problem facts, nor the problem fact collections.
_Therefore the ``__working_solution__`` and the ``__best_solution__`` share the same problem fact instances and the same problem fact list instances._
+
...
Otherwise, if the `working_solution` and `best_solution` are used in different threads (for example a solver thread and a GUI event thread), a race condition can occur.
...
[source,python,options="nowrap"]
----
def best_solution_changed(event: BestSolutionChangedEvent[CloudBalance]) -> None:
