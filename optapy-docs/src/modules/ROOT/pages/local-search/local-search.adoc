...
xref:optapy-configuration/optapy-configuration.adoc#logging[Turn on `trace` logging] to show the decision making in the log:
...
Each time the current solution is better than the last best solution, the current solution is xref:optapy-configuration/optapy-configuration.adoc#cloningASolution[cloned] and referenced as the new best solution.
...
* Random tie breaking does not affect xref:optapy-configuration/optapy-configuration.adoc#environmentMode[reproducibility].
...
Start from an `acceptedCountLimit` that takes a step in less than two seconds. xref:optapy-configuration/optapy-configuration.adoc#logging[Turn on INFO logging] to see the step times.
...
make sure that the planning entities do not include planning variables in their `\\__hash__` methods.
Failure to do so results in runtime exceptions being thrown due to the `\\__hash__` not being constant,
