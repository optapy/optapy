...
solver_config = optapy.config.solver.SolverConfig.createFromXmlFile(pathlib.Path('solverConfig.xml')) \
...
Note that unlike OptaPlanner, *domain classes and the score
director are defined via the Python API*.
...
<solver xmlns="https://www.optaplanner.org/xsd/solver" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
  ...
  <!-- Configure when the solver terminates -->
  <termination>
    ...
  </termination>
  <!-- Configure how the initial solution is built -->
  <constructionHeuristic>
    ...
  </constructionHeuristic>
  <!-- Configure how local search is done -->
  <localSearch>
    ...
  </localSearch>
</solver>
...
----
termination_config = optapy.config.solver.TerminationConfig()
template = optapy.config.solver.SolverConfig.createFromXmlFile(pathlib.Path('solverConfig.xml')) \
    .withSolutionClass(get_class(TimeTable)) \
    .withEntityClasses(get_class(Lesson)) \
    .withConstraintProviderClass(get_class(define_constraints)) \
    .withTerminationConfig(termination_config)
...
*In OptaPlanner, all problem facts and planning entities are plain old Python Objects.* Load them from a database, an XML file, a data repository, a REST service, a noSQL cloud; it doesn't matter.
...
solver_config = optapy.config.solver.SolverConfig.createFromXmlFile(pathlib.Path('solverConfig.xml')) \
    # ...
    .withEntityClasses(get_class(Queen)) \
    # ...
----
...
[NOTE]
====
<<listVariables,List variables>> can also be used for these use cases
====
...
solver_config = optapy.config.solver.SolverConfig.createFromXmlFile(pathlib.Path('solverConfig.xml')) \
    .withSolutionClass(get_class(NQueens)) \
    # ...
----
...
# Returns immediately
solver_job = solver_manager.solve(problemId, problem1);
...
...
class TimeTableService:
    ...
    def solve_batch(self, time_table_id):
