...
Many move types are included out-of-the-box.
// TODO: Uncomment when we support custom python moves
//, but you can also implement custom moves.
...
This move selector only supports <<cacheType,phase or solver caching>> if it doesn't apply on a xref:optapy-configuration/optapy-configuration.adoc#chainedPlanningVariable[chained] variable.
...
----
// TODO: Merge into above when custom comparators are supported
////
      <subPillarSequenceComparatorClass>optapy.examples.nurserostering.domain.shift_assignment_comparator</subPillarSequenceComparatorClass>
////
...
In this case, the entity being operated on must implement the comparisons methods (`\\__lt__`, `\\__gt__`, etc). The size of sub pillars will not be limited in any way.
...
// TODO: Uncomment when custom comparators are supported
////
      <subPillarSequenceComparatorClass>optapy.examples.nurserostering.domain.shift_assignment_comparator</subPillarSequenceComparatorClass>
////
// In this case, the entity being operated on need not be `Comparable`. The given `subPillarSequenceComparatorClass` is used to establish the sequence instead. Also,
...
// TODO: Remove this when custom selectorProbabilityWeightFactory are supported
Custom `selectorProbabilityWeightFactory` functions are currently not supported in OptaPy.
...
Here's how it works.
When the root `MoveSelector` yields a value, a child `MoveSelector` is randomly selected (1), which creates a random `Move` (2, 3, 4) and is then returned (5):
...
Here's how it works: At the start of the phase (or step depending on the ``cacheType``), all moves are created (1) and cached (2). When the root `MoveSelector` is created, the moves are shuffled (3). When the root `MoveSelector` yields a value, the next element in the shuffled list is returned (4):
...
OptaPy currently does not support filtered entity selection, but will in a future version.
...
OptaPy currently does not support filtered value selection, but will in a future version.
...
** ``DECREASING_DIFFICULTY``: Sorts the planning entities according to decreasing xref:optapy-configuration/optapy-configuration.adoc#planningEntityDifficulty[planning entity difficulty]. Requires that planning entity difficulty is annotated on the domain model.
+
[source,xml,options="nowrap"]
----
    <entitySelector>
...
** ``INCREASING_STRENGTH``: Sorts the planning values according to increasing xref:optapy-configuration/optapy-configuration.adoc#planningValueStrength[planning value strength]. Requires that planning value strength is annotated on the domain model.
+
[source,xml,options="nowrap"]
----
    <valueSelector>
...
==== Sorted selection by comparison function

OptaPy currently does not support sorted selection by comparison function, but will in a future version.

////
TODO: Uncomment when supported
An easy way to sort a `Selector` is with a plain old comparison function:

[source,python,options="nowrap"]
----
def compare_cloud_process_difficulty(a: CloudProcess, b: CloudProcess) -> int:
    # compare required_multiplicand, then process_id
    a_value_tuple = (a.required_multiplicand, a.process_id)
    b_value_tuple = (b.required_multiplicand, b.process_id)
    if a_value_tuple < b_value_tuple:
        return -1
    elif a_value_tuple > b_value_tuple:
        return 1
    else:
        return 0
----
...
----
    <entitySelector>
      <cacheType>PHASE</cacheType>
      <selectionOrder>SORTED</selectionOrder>
      <sorterComparatorClass>compare_cloud_process_difficulty</sorterComparatorClass>
...
OptaPy currently does not support sorted selection by weight factory, but will in a future version.
////
If you need the entire solution to sort a ``Selector``, use a `SelectionSorterWeightFactory` instead:

[source,python,options="nowrap"]
----
def difficulty_weight_factory(solution: Solution, selection: SelectionType) -> Comparable:
    ...
----

[source,python,options="nowrap"]
----
def queen_difficulty_weight_factory(n_queens, queen):
    distance_from_middle = calculate_distance_from_middle(n_queens.n, queen.column_index)
    return (distance_from_middle, queen.column_index)
----
...
OptaPy currently does not support sorted selection by selection sorter, but will in a future version.
////
TODO: Uncomment when supported
...
[source,python,options="nowrap"]
----
def sort(score_director: ScoreDirector[Solution], selectionList: list[SelectionType]) -> None:
    ...
----
...
OptaPy currently does not support nearby selection, but will in a future version.
...
OptaPy currently does not support custom moves, but will in a future version.
