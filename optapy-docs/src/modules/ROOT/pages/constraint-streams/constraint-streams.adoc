...
Using Python's iterable operations, we could implement an xref:score-calculation/score-calculation.adoc#easyJavaScoreCalculation[easy score calculator]
...
This constraint stream iterates over all instances of class `Shift` in the xref:optapy-configuration/optapy-configuration.adoc#problemFacts[problem facts] and
xref:optapy-configuration/optapy-configuration.adoc#planningEntity[planning entities] in the <<planningProblemAndPlanningSolution,planning problem>>.
...
is in a xref:optapy-configuration/optapy-configuration.adoc#problemFacts[problem fact collection]
or a xref:optapy-configuration/optapy-configuration.adoc#planningEntitiesOfASolution[planning entity collection]
...
`from_()` behaves unexpectedly for xref:optapy-configuration/optapy-configuration.adoc#nullablePlanningVariable[nullable variables].
