...
However, OptaPy has a system to prevent CPU starving of
...
*This parameter basically specifies how many CPU cores are donated to OptaPy.*
...
* `UNLIMITED`: Allow OptaPy to occupy all CPU cores, do not avoid hogging.
Useful if a no hogging CPU policy is configured on the OS level.
* `AUTO` (default): Let OptaPy decide how many CPU cores to occupy. This formula is based on experience.
...
unless there is an OS specific policy in place to avoid OptaPy from hogging all the CPU processors.
====
////